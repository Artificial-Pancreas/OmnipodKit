...
public class OmniPumpManager: RileyLinkPumpManager {
    ...
    init(
        ...
        localizedTitle = state.podType.description
    ...
    private func setState(_ changes: (_ state: inout OmniPumpManagerState) -> Void) {
    ...
    func updateRLConnectionStatus() {
        guard podComms is ErosPodComms else {
            return
        }
...
extension OmniPumpManager {
    ...
    private func basalDeliveryState(for state: OmniPumpManagerState, at date: Date = Date()) -> PumpManagerStatus
        .BasalDeliveryState
    {
    ...
    private func handleSilencePodEnd(session: PodCommsSession) {
        if let silencePodEnd = state.silencePodEnd, Date() >= silencePodEnd {
            log.default(
                "@@@ handleSilencePodEnd: end time %{public}@ reached, disabling silence mode",
                String(describing: silencePodEnd)
            )
            ...
            doSetSilencePod(session: session, silencePod: false, silencePodEnd: nil) { error in
                if let error = error {
                    self.log.default(
                        "@@@ handleSilencePodEnd: disable silence mode failed: %{public}@",
    ...
    func buildPumpLifecycleProgress(for _: OmniPumpManagerState) -> PumpLifecycleProgress? {
        switch podCommState {
        case .active:
            if shouldWarnPodEOL,
    ...
    var podType: PodType {
        set {
            ...
            if state.podType == unknownOmnipodType {
                ...
                log.info(
                    "Changing OmniPumpManager podType from %{public}@ to %{public}@",
                    state.podType.briefName,
                    newValue.briefName
                )
            ...
            lockedPodComms = Locked(podComms)
            localizedTitle = newValue.description // set the OmniSettingsView title
            ...
            prepForNewPod() // reset the Id's as appropriate for the new pod type
        }
        get {
            state.podType
        }
    ...
    private func prepForNewPod() {
        ...
        setState { state in
            ...
            case dashType,
                ...
                (state.controllerId, state.podId) = nextIds(
                    podType: podType,
                    controllerId: state.controllerId,
                    podId: state.podId
                )
                self.log.info(
                    "@@@ prepForNewPod: set controllerId to 0x%08X and podId to 0x%08X",
                    state.controllerId,
                    state.podId
                )
    ...
    func forgetPod(completion: @escaping () -> Void) {
        podComms.handleDiscardedPodDosing(podTime: podTime, reservoirLevel: reservoirLevel?.rawValue)
    ...
    func forgetBluetoothManager() {
        if let blePodComms = lockedPodComms.value as? BlePodComms {
    ...
    // MARK: Testing
        ...
        private func jumpStartPod(
            ...
            var podState = PodState(
                ...
                setupUnitsDelivered: Pod.primeUnits
            )
        ...
        guard let insulinType = insulinType else {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + .seconds(2)) {
                if self.state.podType.usesRileyLink {
                    let address: UInt32 = self.state.podState?.address ?? 0x1F1F_1F1F
                    self.jumpStartPod(
                        address: address,
                        lotNo: 135_601_809,
                        lotSeq: 0_800_525,
                        startDate: mockStartDate,
                        mockFault: mockFaultDuringPairing
                    )
                } else {
                    let address: UInt32 = self.state.podState?.address ?? 0x1717_1717
                    self.jumpStartPod(
                        address: address,
                        lotNo: 135_601_809,
                        lotSeq: 0_800_525,
            ...
            if needsPairing {
                ...
                if let erosPodComms = podComms as? ErosPodComms {
                    ...
                    if state.pairingAttemptAddress == nil {
                        lockedState.mutate { state in
                            ...
                            state.pairingAttemptAddress = upperByte | (arc4random() & 0x000F_FFFF)
                        }
                    }
                ...
                } else if let blePodComms = podComms as? BlePodComms {
                ...
                runSession(withName: "Prime pod") { result in
                    // Calls completion
                    primeSession(result)
                }
            }
    ...
    func insertCannula(completion: @escaping (Result<TimeInterval, OmniPumpManagerError>) -> Void) {
            ...
            runSession(withName: "Insert cannula") { result in
                switch result {
                case let .success(session):
                    ...
                    do {
                        ...
                        if self.state.defaultExpirationReminderOffset > 0 {
                            ...
                            alerts
                                .append(.expirationReminder(
                                    offset: self.podTime,
                                    absAlertTime: expirationReminderTime,
                                    silent: silencePod
                                ))
                        }
    ...
    func checkCannulaInsertionFinished(completion: @escaping (OmniPumpManagerError?) -> Void) {
            ...
            completion(nil)
        #else
            runSession(withName: "Check cannula insertion finished") { result in
                switch result {
                case let .success(session):
                    do {
    ...
    func resumingPodSetup() {
        ...
        guard state.podState?.setupProgress.isPaired == true else {
            log.debug("### Pod setup resume skipping getStatus as not yet paired")
            return
        }

        runSession(withName: "Resuming pod setup") { result in
            switch result {
            case let .success(session):
    ...
    private func runSession(withName name: String, _ block: @escaping (_ result: PodComms.SessionRunResult) -> Void) {
        ...
        } else if let erosPodComms = podComms as? ErosPodComms {
            let device = rileyLinkDeviceProvider.firstConnectedDevice
    ...
    private func handlePodUpdatesAsNeeded(session: PodCommsSession, canOptimize: Bool) -> StatusResponse? {
        ...
        let timeSinceLastResponse = -(state.podState?.podTimeUpdated ?? .distantPast).timeIntervalSinceNow
        let status: StatusResponse?
        if canOptimize, timeSinceLastResponse < optimizeInterval {
            log.debug("### skipping getStatus() with last status %@ ago", timeSinceLastResponse.timeIntervalStr)
            status = nil
        } else {
    ...
    )
    {
        ...
        runSession(withName: "Get pod status") { result in
            do {
                switch result {
                case let .success(session):
    ...
    func acknowledgePodAlerts(_ alertsToAcknowledge: AlertSet, completion: @escaping (_ alerts: AlertSet?) -> Void) {
        ...
        runSession(withName: "Acknowledge Alerts") { result in
    ...
    func setTime(completion: @escaping (OmniPumpManagerError?) -> Void) {
        let timeZone = TimeZone.currentFixed
            ...
            runSession(withName: "Set time zone") { result in
                switch result {
                case let .success(session):
                    do {
                        self.handleSilencePodEnd(session: session)
    ...
    func setBasalSchedule(_ schedule: BasalSchedule, completion: @escaping (Error?) -> Void) {
        ...
        runSession(withName: "Save Basal Profile") { result in
            do {
                switch result {
                case let .success(session):
    ...
    func deactivatePod(completion: @escaping (OmniPumpManagerError?) -> Void) {
            ...
            runSession(withName: "Deactivate pod") { result in
                switch result {
                case let .success(session):
                    do {
    ...
    func setConfirmationBeeps(newPreference: BeepPreference, completion: @escaping (OmniPumpManagerError?) -> Void) {
        ...
        runSession(withName: name) { result in
            switch result {
            case let .success(session):
                if let error = self.tryToValidateComms(session: session) {
                    completion(.communication(error))
    ...
    private func updateManualInsulinBeepState(session: PodCommsSession, enabled: Bool) -> Error? {
        guard hasUnfinalizedManualTempBasal || hasUnfinalizedManualBolus else {
    ...
    func setSilencePod(silencePod: Bool, silencePodEnd: Date?, completion: @escaping (OmniPumpManagerError?) -> Void) {
        ...
        if let silencePodEnd = silencePodEnd, !(silencePodEnd > Date() && silencePod) {
            ...
            let justUpdateState = !hasActivePod || state.silencePod == silencePod
        #endif
    ...
    private func doSetSilencePod(
        session: PodCommsSession,
        silencePod: Bool,
        silencePodEnd: Date?,
    ...
    )
    {
        guard let configuredAlerts = state.podState?.configuredAlerts,
              ...
              let reservoirLevel = state.podState?.lastInsulinMeasurements?.reservoirLevel?.rawValue
        else {
        ...
        if !beepPreference.shouldBeepForManualCommand {
...
extension OmniPumpManager: PumpManager {
    ...
    func suspendDelivery(withSuspendReminders suspendReminder: TimeInterval? = nil, completion: @escaping (Error?) -> Void) {
        ...
        runSession(withName: "Suspend") { result in
    ...
    public func resumeDelivery(completion: @escaping (Error?) -> Void) {
        ...
        runSession(withName: "Resume") { result in
    ...
    public func enactBolus(
        ...
        runSession(withName: "Bolus") { result in
            let session: PodCommsSession
            switch result {
            case let .success(s):
                session = s
    ...
    public func cancelBolus(completion: @escaping (PumpManagerResult<DoseEntry?>) -> Void) {
        ...
        runSession(withName: "Cancel Bolus") { result in
    ...
    public func enactTempBasal(
        ...
        guard hasActivePod, let podState = state.podState else {
            completion(.configuration(OmniPumpManagerError.noPodPaired))
            return
        }
        ...
        runSession(withName: "Enact Temp Basal") { result in
    ...
    func updateExpirationReminder(
        ...
        runSession(withName: "Update Expiration Reminder") { result in
    ...
    func updateLowReservoirReminder(_ value: Int, completion: @escaping (OmniPumpManagerError?) -> Void) {
        ...
        runSession(withName: "Program Low Reservoir Reminder") { result in
    ...
    func updateLowReservoirDefaultReminder(_ value: Int, completion: @escaping (OmniPumpManagerError?) -> Void) {
        let supportedValue = min(max(0, Double(value)), Pod.maximumReservoirReading)
        log.default("Set Default Low Reservoir Reminder to %d U", value)
        defaultLowReservoirReminderValue = supportedValue
        completion(nil)
    }
    ...
    private func silenceAcknowledgedAlerts() {
        ...
        if let alert = state.alertsWithPendingAcknowledgment.first {
            if let slot = alert.triggeringSlot {
                runSession(withName: "Silence already acknowledged alert") { result in
                    switch result {
                    case let .success(session):
                        do {
            ...
            delegate?
                .issueAlert(Alert(
                    identifier: Alert.Identifier(
                        managerIdentifier: OmniPumpManager.podAlarmNotificationIdentifier,
...
extension OmniPumpManager: PodCommsDelegate {
    ...
    func podCommsDidEstablishSession(_ podComms: PodComms) {
        guard podComms.podState?.isSetupComplete == true else {
            log.debug("### Skipping post-connect processing with incomplete setup")
            return
        }

        runSession(withName: "Post-connect status fetch") { result in
            switch result {
            case let .success(session):
                _ = self.handlePodUpdatesAsNeeded(session: session, canOptimize: true)
            case .failure:
...
public extension OmniPumpManager {
    func acknowledgeAlert(alertIdentifier: Alert.AlertIdentifier, completion: @escaping (Error?) -> Void) {
        guard hasActivePod, !state.activeAlerts.isEmpty else {
        ...
        for alert in state.activeAlerts {
            if alert.alertIdentifier == alertIdentifier || alert.repeatingAlertIdentifier == alertIdentifier {
                ...
                if let slot = alert.triggeringSlot {
                    ...
                    runSession(withName: "Acknowledge Alert") { result in
                        switch result {
                        case let .success(session):
                            self.handleSilencePodEnd(session: session)
