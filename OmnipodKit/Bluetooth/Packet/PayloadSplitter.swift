...
class PayloadSplitter {
    ...
    func splitInPackets() -> [BlePacket] {
        ...
        let rest = UInt8(
            (payload.count - middleFragments * layout.middlePacketCapacity) - layout
        ...
        if middleFragments > 0 {
            for i in 1 ... middleFragments {
                let p = payload
                    .subdata(
                        in: (layout.firstPacketCapacityWithMiddlePackets + (i - 1) * layout.middlePacketCapacity) ..<
        ...
        ret.append(
            LastBlePacket(
                ...
                payload: payload
                    .subdata(
                        in: middleFragments * layout.middlePacketCapacity + layout
                            .firstPacketCapacityWithMiddlePackets ..< middleFragments * layout.middlePacketCapacity + layout
                            .firstPacketCapacityWithMiddlePackets + end
                    ),
                crc32: crc32
            )
        )
            ...
            ret.append(
                LastOptionalPlusOneBlePacket(
                    ...
                    payload: payload
                        .subdata(
                            in: middleFragments * layout.middlePacketCapacity + layout
                                .firstPacketCapacityWithMiddlePackets + layout.lastPacketCapacity ..< payload.count
                        ),
