...
private var bleDebugEnabled = false
...
extension PeripheralManager {
    ...
    func sendMessagePacket(_ message: MessagePacket, _ forEncryption: Bool = false) -> SendMessageResult {
        ...
        do {
            if podType.isDash {
                ...
                log.bleDebug(
                    "[sendMessagePacket] Skipping RTS/CTS, writing data directly. peripheral state=%{public}@",
            ...
            log.bleDebug(
                "[sendMessagePacket] Split payload into %{public}d packet(s), total payload %{public}d bytes",
                packets.count,
            ...
            for (index, packet) in packets.enumerated() {
                ...
                log.bleDebug(
                    "[sendMessagePacket] Writing data packet %{public}d/%{public}d (%{public}d bytes)... peripheral state=%{public}@",
                    index + 1,
                    packets.count,
                    packetData.count,
                    String(describing: peripheral.state)
                ...
                log.bleDebug(
                    "[sendMessagePacket] Data packet %{public}d/%{public}d written. Peeking for NACK...",
                    index + 1,
                    packets.count
                )
                try peekForNack()
            ...
            log.bleDebug(
                "[sendMessagePacket] All packets written. Waiting for SUCCESS... peripheral state=%{public}@",
                String(describing: peripheral.state)
            )
            try waitForCommand(PodCommand.SUCCESS, timeout: 5)
            log.bleDebug(
                "[sendMessagePacket] SUCCESS received. peripheral state=%{public}@",
                String(describing: peripheral.state)
            )
        } catch {
            log.error(
                "[sendMessagePacket] Error (didSend=%{public}@): %{public}@. peripheral state=%{public}@",
                String(describing: didSend),
    ...
    func readMessagePacket() throws -> MessagePacket? {
        ...
        do {
            if podType.isDash {
                ...
                log.bleDebug(
                    "[readMessagePacket] Skipping RTS/CTS, waiting for data. peripheral state=%{public}@",
                    String(describing: peripheral.state)
                )
            }
            ...
            log.bleDebug(
                "[readMessagePacket] Waiting for first data packet (seq 0)... peripheral state=%{public}@",
                String(describing: peripheral.state)
            )
            ...
            log.bleDebug(
                "[readMessagePacket] Expecting %{public}d more fragment(s) (fullFragments=%{public}d, oneExtra=%{public}@)",
                totalFragments,
                joiner.fullFragments,
                String(describing: joiner.oneExtraPacket)
            )
            ...
            if joiner.fullFragments > 0 {
                for i in 1 ... joiner.fullFragments {
                    expected += 1
                    log.bleDebug(
                        "[readMessagePacket] Waiting for fragment %{public}d (seq %{public}d)... peripheral state=%{public}@",
                        i,
                        expected,
                        String(describing: peripheral.state)
                    )
            ...
            if joiner.oneExtraPacket {
                expected += 1
                log.bleDebug(
                    "[readMessagePacket] Waiting for extra fragment (seq %{public}d)... peripheral state=%{public}@",
                    expected,
                    String(describing: peripheral.state)
                )
                let packet = try waitForData(sequence: expected, timeout: 5)
            ...
            log.bleDebug(
                "[readMessagePacket] All fragments received, total payload %{public}d bytes. Sending SUCCESS...",
                fullPayload.count
            )
            ...
            log.bleDebug(
                "[readMessagePacket] Message parsed successfully. peripheral state=%{public}@",
                String(describing: peripheral.state)
            )
        } catch {
            log.error(
                "[readMessagePacket] Error reading message: %{public}@. peripheral state=%{public}@",
                String(describing: error),
                String(describing: peripheral.state)
            )
            if let error = error as? PeripheralManagerError, error.isSymptomaticOfUnresponsivePod {
                if peripheral.state == .connected {
                    ...
                    log.error(
                        "[readMessagePacket] Pod already not connected (state=%{public}@), skipping disconnect",
                        String(describing: peripheral.state)
                    )
                }
            } else {
        ...
        while true {
            ...
            if !cmdQueue.isEmpty {
                ...
                if command.rawValue == value[0] {
                    log.bleDebug(
                        "waitForCommand: got expected 0x%{public}02x, full data=%{public}@ (%{public}d bytes)",
                        command.rawValue,
                ...
                if value[0] == PodCommand.PAIR_STATUS.rawValue {
                    log.bleDebug(
                        "waitForCommand: skipping intermediate PAIR_STATUS (0x08), data=%{public}@, waiting for 0x%{public}02x",
                        value.hexadecimalString,
                        command.rawValue
                    )
                    queueLock.unlock()
    ...
    func sendData(_ value: Data, timeout: TimeInterval) throws {
        ...
        guard let characteristic = peripheral.getDataCharacteristic(profile: profile) else {
            log.error(
                "Unable to get characteristic... peripheral status: %{PUBLIC}@",
                String(describing: peripheral.state)
            )
            throw PeripheralManagerError.notReady
        }
